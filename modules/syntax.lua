...
function Syntax:createTokenInstance(ttype, value)
    ...
    dassert(token, "Could not find token for ttype=",ttype)
    local copy = {
        ["ttype"]    = token.ttype,
        ["ptype"]    = token.ptype,
...
function Syntax:convertTokenNumber(token)
    dassert(token.ptype == "NUMBER", "Enumerator does not equal a number type")
    ...
    elseif token.ttype == "HEX_LITERAL" then
        ...
        dassert(val, "Could not convert hex='", token.value,
                     "' to a number")
        value = tonumber(val, 16)
    elseif token.ttype == "OCTAL_LITERAL" then
        local val = oct_rgx:match(token.value)
        dassert(val, "Could not convert octal='", token.value,
                     "'' to a number")
        value = tonumber(val, 8)
    end

    dassert(value, "Could not convert '", token.value,
                   "'' to a number")
