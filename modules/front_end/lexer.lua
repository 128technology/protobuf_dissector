...
local M = { debug_level  = DEBUG }

local dprint = function() end
local dprint2 = function() end
local function resetDebugLevel()
    if M.debug_level > debug_level.DISABLED then
        dprint = function(...)
            info(table.concat({"Lua: ", ...}," "))
        end

        if M.debug_level > debug_level.LEVEL_1 then
            dprint2 = dprint
        end
    else
        dprint = function() end
...
function M:generateVariableMatchPatterns()
    local tbl = {}
...
function M:generateStringMatchPatterns()
    local tbl = {}
...
function M:generateSkipPatterns()
    local tbl = {}
    for _, category in ipairs({"COMMENT", "STRING"}) do
        for _, token in ipairs(self.syntax.token_info[category]) do
            if token.skip then
...
function M:makeTokenPatternString()
...
function M:removeComments(chunk)
    local result = ""
    local prev_stop = 1
...
function M:matchVariable(value)
...
function M:skipChunk(chunk, begin, ttype)
