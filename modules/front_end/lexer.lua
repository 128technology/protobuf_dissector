...
function Lexer:tokenize(chunk)
    ...
    while start do
        ...
        dprint2("tokenize: either='", either, 
                "', skip='", skip,
                "', symbol='", symbol,
                "', word='", word,
                "', variable='", variable, "'")
        ...
        if skip then
            ...
            dassert(stop, "Could not skip ahead with: ", ttype)
        elseif symbol or word then
            ...
            dassert(ttype, "Matched fixed-type word '", fixed, "' did not match a token")
        else
        ...
        dassert(ttype, "Could not match word '", either, "' to a token")
