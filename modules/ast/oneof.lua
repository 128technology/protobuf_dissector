...
function OneofStatement.parse(st, namespace)
    assert(AstFactory.verifyTokenTTypes("Oneof", st, "ONEOF", "IDENTIFIER", "BRACE_BLOCK"))
...
function OneofStatement:analyze(fields, tags)
    for _, object in ipairs(self.value) do
        object:analyze()
        assert(not tags[object:getTag()], "Two fields with same tag number in Message/Oneof")
