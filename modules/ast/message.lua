...
function MessageStatement:analyze()
    ...
    for _, object in ipairs(self.value) do
        ...
        if not ignore_types[object:getType()] then
            assert(not tags[object:getTag()], "Two fields with same tag number '" .. tostring(object:getTag()) .. "' in Message: " .. self:getName())
...
function MessageStatement:createProto()
    ...
    if not self.proto then
        --print("Creating Proto for " .. self.namespace:getFullName() .. " with name " .. self:getName())
