...
setmetatable( MessageStatement, { __index = ast.StatementBase } ) -- make it inherit from StatementBase
ast:registerClass("MESSAGE", "MessageStatement", MessageStatement, true)


function MessageStatement.preParse(st)
    assert(ast.verifyTokenTTypes("Message", st, "MESSAGE", "IDENTIFIER", "BRACE_BLOCK"))
...
function MessageStatement:postParse(st, id, namespace)
    ...
    for _,tokens in ipairs(st[3].value) do
        value[#value+1] = ast:dispatchBodyStatement(ns, tokens)
    end
...
function MessageStatement:new(namespace, st)
    ...
    local new_class = ast.StatementBase:new("MESSAGE", id)
...
function MessageStatement:createTagTable()
    if not self.tag_table then
        ...
        for _, object in ipairs(self.value) do
            pfields[#pfields+1] = object:createProtoField()
            tags[object:getTag()] = object:getDecoder()
        end
...
function MessageStatement:getDecoder(pfield, tag, name)
    ...
    return decoder.MessageDecoder.new(pfield, tag, name, self)
end
...
function MessageStatement:createDissector()
    ...
    assert(proto, "Programming error: Proto not created")
    assert(decoder, "Programming error: self decoder not created")
    ...
    proto.dissector = function(tvbuf,pktinfo,root)
        return back_end.Decoder:new(tvbuff, pktinfo, root):dissect(decoder)
    end
end
...
function MessageStatement:createProto()
    ...
    local proto = Proto.new(self.namespace:getFullName(), self.id:display())

    self:createTagTable()

    -- register the ProtoFields
    proto.fields = self.pfields
