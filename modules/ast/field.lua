...
function FieldStatement.parse(st, one_of_name)
    local idx, id, value
...
function FieldStatement.new(namespace, st, one_of_name)
    local id, value, tag = FieldStatement.parse(st, one_of_name)
    local new_class = StatementBase.new("FIELD", id, namespace, value)
    if one_of_name then
        -- this Field occurs in a oneof statement
        new_class["oneof"] = one_of_name
    else
...
function FieldStatement:getDecoder()
    if not self.decoder then
        self.decoder = self.value:getDecoder(self:getProtoField(), self:getTag(), self:getName())
    end
    return self.decoder
end
