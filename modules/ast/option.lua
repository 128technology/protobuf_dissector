...
setmetatable( OptionStatement, { __index = ast.StatementBase } ) -- inherit from StatementBase
ast:registerClass("OPTION", "OptionStatement", OptionStatement, true)

function OptionStatement.parse(st)
    assert(ast.verifyTokenTTypes("Option", st, "OPTION", false, false))

    local id, idx = "", 2
    if st[idx].ttype == "PARENS_BLOCK" then
        local subtbl = st[2].value
        assert(#subtbl > 0, "OPTION fullIdent not given")
        id = ast.ScopedIdentifier:new(subtbl, 1)
        idx = idx + 1
        ...
        id, idx = ast.ScopedIdentifier:new(st, idx)
    end
    ...
    local value = ast.serializeRemaining(st, idx)
    return id, value
end
...
function OptionStatement:new(namespace, st)
    ...
    local new_class = ast.StatementBase:new("OPTION", id, namespace, value) -- the new instance
