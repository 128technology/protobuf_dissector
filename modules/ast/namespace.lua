...
ast:registerClass(nil, "Namespace", Namespace)


function Namespace:new(name, parent)
    local full_name
    if orig_parent and parent:getFullName() then
...
function Namespace:buildAst(parse_tree)
    ...
    for _, statement in ipairs(parse_tree) do
        if statement[1].ttype ~= "IMPORT" and statement[1].ttype ~= "PACKAGE" then
            t[#t+1] = ast:buildStatement(self, statement)
        end
    end
end
...
function Namespace:addDeclaration(id, object)
    ...
    assert(ctype == "MESSAGE" or ctype == "ENUM", "given wrong class type: " .. ctype)

    local name = id:display()

    assert(not self.children[name], "Namespace already has name: " .. name)
    local ns = self:new(name, self)
    self.children[name] = ns

    assert(not self.decls[name], "Namespace already has declaration: " .. name)
...
function Namespace:createChild(name)
    assert(not self.children[name], "Namespace already has package name: " .. name)
    assert(not self.decls[name], "Namespace already has declared name: " .. name)
    local ns = self:new(name, self)
    self.children[name] = ns
    return ns
end
...
function Namespace:findOrCreateNode(scoped_id, is_find, get_decl)
    ...
    assert(name, "Namespace:findOrCreateAbsoluteNode(): Could not get initial scoped name")
    ...
    if get_decl then
        ...
        ns = child:getParent()
        assert(ns, "Could not find scoped identifier '" .. scoped_id:display() .. "'")
...
function Namespace:findOrCreateAbsolute(scoped_id)
    ...
    assert(ns, "Namespace:findOrCreateAbsolute(): could not find root")
...
function Namespace:findAbsolute(scoped_id)
    local ns = self:getRoot()
    assert(ns, "Namespace:findAbsolute(): could not find root")
...
function Namespace:findAbsoluteDeclaration(scoped_id)
    local ns = self:getRoot()
    assert(ns, "Namespace:findAbsoluteDeclaration(): could not find root")
...
function Namespace:findRelativeDeclaration(scoped_id)
    ...
    assert(name, "Namespace:findRelativeDeclaration(): Could not get initial scoped name")

    -- find first name portion's parent
    local parent = self:findParent(name)
    assert(parent, "Could not find first portion of scoped identifier '" .. scoped_id:display() .. "'")
...
function Namespace:analyze()
    ...
    assert(#self.ast == 0, "Programming error: some statements in AST not analyzed")
end
...
function Namespace:createProtocols(proto_table)
    for name, object in pairs(self.decls) do
        if object:getType() == "MESSAGE" then
            proto_table[name] = object:createProto()
        end
    end
