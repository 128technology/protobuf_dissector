...
local front_end = require "front_end.front_end"
local back_end  = require "back_end.back_end"
...
function compiler:compile(proto_filenames, do_debug, pfunc)
    ...
    local global_ast = front_end:execute(proto_filenames, do_debug, pfunc)
    assert(global_ast, "Front-end failed to return an AST")

    print("The AST:")
    display(global_ast)

    local decode_table = back_end:execute(global_ast)
