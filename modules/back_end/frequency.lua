...
-- tags that are required or oneof, with the table values also being pointers
-- to the tag_counts table entries. It uses this to verify each required tag
-- has a count > 0.
--
...
function Frequency:addTagValueToTable(table_name, tag, value)
    ...
    assert(not self[table_name][tag], "Programming error: tag already exists in " .. table_name .. " table: " .. tag)
...
function Frequency:addRequired(tag, counter)
    return self:addTagValueToTable("required", tag, counter)
end
...
function Frequency:init(tag_dispatch_tbl)
    for tag, object in pairs(tag_dispatch_tbl) do
        ...
        if label == "ONEOF" then
            ...
            assert(oneof_name, "Programming error: could not get oneof name for object: " .. object:getName())
...
function Frequency:checkTag(tag)
    local counter = tags[tag]
    assert(counter, "Programming error: counter not found for tag: " .. tag)
    ...
    if count > 1 and self.single[tag] then
        local label = labels[tag]
        assert(label, "Programming error: label not found for tag: " .. tag)
