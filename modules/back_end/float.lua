...
setmetatable( FloatDecoder, { __index = Base } ) -- make it inherit from Base



function FloatDecoder.new(pfield, tag, name)
    local new_class = Base.new("FLOAT", pfield, tag, name)
...
function FloatDecoder:decode(decoder)
    ...
    return decoder:addFieldStruct(self.pfield, "<f", 4)
end
