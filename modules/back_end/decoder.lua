...
function Decoder:checkFrequency(tree)
    local ok, expert_type = self.frequency:checkTag(self.tag)
    if not ok then
        -- TODO: expert info
    end
end
...
local pfield_info = PFieldInfo:getFields()
...
function Decoder:addFieldInfo(tree, size)
    ...
    local key_tree = tree:add(pfield_info.key, self.tvbuf(self.field_start, self.key_hdr_size))
    key_tree:add(pfield_info.tag, self.tvbuf(self.field_start, self.key_hdr_size), self.tag)
    key_tree:add(pfield_info.wiretype, self.tvbuf(self.field_start,1), self.last_wiretype)

    -- add length delimiter info if there is one
    if self.last_wtype == "LENGTH_DELIMITED" then
        tree:add(pfield_info.length_delimiter,
                 self.tvbuf(self.field_start + self.key_hdr_size, self.length_size),
                 size)
    end

    tree:add(pfield_info.length, size):set_generated()

    tree:add(pfield_info.value, self.tvbuf(self.field_start + self.key_size, size))
