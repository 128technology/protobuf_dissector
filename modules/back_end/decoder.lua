...
function Decoder:getKey()
    print("Decoder:getKey() called")

    local tag, wiretype, size = varint:decodeKey(self.raw, self.cursor, self.limit)
    print("Got tag=" .. tostring(tag) .. ", wiretype=" .. tostring(wiretype) .. ", size=" .. tostring(size))
    if not tag then
...
function Decoder:decodeTags(tags)
    ...
    while self.limit > 0 do
        ...
        -- addFieldXXX() call
        local size, length_size = self:getWtypeSize(wtype)
        if not size then return end

        self.key_size = key_size + length_size
        ...
        if self:enterRecursion() then
            ...
            if not object:decode(self, tag, wtype) then
...
function Decoder:addFieldVarintSigned32(pfield)
    ...
    local tree = self.tree:add(pfield, self.tvbuf(self.field_start + self.key_size, size), value)
...
function Decoder:addFieldVarint64(pfield)
    local value, size = varint:decode64(self.raw, self.cursor, self.limit)
...
function Decoder:addField(pfield)
    return self.tree:add(pfield, self.tvbuf(self.field_start, self.limit))
end


-- ads the given ProtoField for limit size, advances
function Decoder:addFieldBytes(pfield)
    local tree = self.tree:add(pfield, self.tvbuf(self.cursor, self.limit))
