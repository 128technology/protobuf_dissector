...
--ast:registerClass(nil, "Decoder", Decoder)
...
function Decoder.new(tvbuf, pktinfo, root)
    local new_class = {  -- the new instance
        ...
        -- occurs for Message/Group, and popped when it returns from it
        ["tree"] = root,
...
-- off
function Decoder:dissect(message)
    message:dissect(self)
end
...
function Decoder:dissectTags(tags)
...
function Decoder:addFieldStruct(pfield, fmt, size)
    ...
    self.tree:add(pfield, Struct.unpack(fmt, self.raw, self.cursor))
...
function Decoder:addProto(pfield, name, start, size, separator)
    ...
    -- adjust start to cursor position
    self.tree = old_tree:add(pfield, self.tvbuf(start + self.cursor,size))
